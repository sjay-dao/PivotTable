...
function displayPivotTblData(){
    $.ajax({	
        ...
        success: function(data) {
           ...
           $("thead").html("<th>Sales Person</th>"); 
           arr_item.forEach((value, index)=>{
           ...
           $("#tbl_body").html("");
            ...
            let  col_cnt=0;
            let row_cnt=-1;
            let pivot_data_cnt = 0;
            let current_seller = "xxxxx";
            do{
                for(y=0; y<arr_item.length; y++){
                    if(current_seller != pivot_data[pivot_data_cnt][1]){
                        current_seller = pivot_data[pivot_data_cnt][1];
                        row_cnt++;
                         $("#tbl_body").append("<tr id='tr"+row_cnt+"'><td>"+pivot_data[pivot_data_cnt][1]+"</td></tr>");
                        break;
                    }

                    if(pivot_data[pivot_data_cnt][2] != arr_item[y]){// if the data inside pivot has no item within the seller then it just hop
                        $("#tr"+row_cnt).append("<td> ---- </td>");
                    }
                    else{
                        $("#tr"+row_cnt).append("<td>"+pivot_data[pivot_data_cnt][3]+"</td>");
                        pivot_data_cnt++;
                    }
                    // console.log( pivot_data[scnt][2] + pivot_data[scnt][3]);
                }
                // console.log("scnt" + scnt);
            }while(pivot_data_cnt < pivot_data.length);
